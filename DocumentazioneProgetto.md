...
Questo è possibile perché in una SPA, il server invia al client tutto il codice necessario al funzionamento dell'applicazione web, cioé una pagina HTML, CSS e una serie di file JavaScript che si occupano di interagire con l'utente ed aggiornare i vari componenti della pagina dinamicamente, senza dover chiedere al server di rerenderizzare l'intera pagina come avviene in pagine dinamiche tradizionali scritti in linguaggi come PHP.
...
React è una libreria JavaScript sviluppata da Facebook nel 2011 che ci permette di creare intefacce utente componibili, ovvero formati da vari "componenti" riutilizzabili che possono rappresentare dati che cambiano nel tempo, aggiornandosi automaticamente, infatti ogni componente possiede un proprio stato interno, e al suo cambiamento si aggiornerà.

Tramite i componenti si possono creare diverse viste in base alla route (utilizzando un modulo di React chiamato React-Router-DOM), ovvero l'URL inserito dall'utente, simulando lo spostamento tra pagine diverse, renderizzando solo i componenti che cambiano, infatti solitamente nelle applicazioni React abbiamo diversi componenti che rimangono costanti come ad esempio la barra di navigazione.
