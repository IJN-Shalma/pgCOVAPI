...
async function updateProvince() {
    ...
    if (newData.length > 0) {
        Provincia.insertMany(newData)
            .then(() => console.log("Province - Updated"))
            .catch((err) => console.log(err))

    }
    else {
        console.log("Province - Already Up to Date")
    }
}

module.exports = { updateRegioni, updateNazioni, updateProvince };